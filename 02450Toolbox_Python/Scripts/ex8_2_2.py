...
model = lambda: torch.nn.Sequential(
                    # a function that maps M features to H hidden units
                    torch.nn.Linear(M, n_hidden_units), # M features to H hiden units
                    # 1st transfer function, either Tanh or ReLU:
                    torch.nn.Tanh(),                            #torch.nn.ReLU(),
                    torch.nn.Linear(n_hidden_units, 1), # H hidden units to 1 output neuron
                    # to train a binary classifier we need a suitable transfer function, Sigmoid()
                    torch.nn.Sigmoid() # final tranfer function
                    )
# By using a lambda function we can get a new copy of the network each time we
# call model(), which we need since we’re doing K-fold cross-validation and multiple
# replicates within each fold.

# Neural Network are trained by iterativel reducing a cost function/loss
...
for k, (train_index, test_index) in enumerate(CV.split(X,y)): 
    ...
    y_sigmoid = net(X_test) # activation of final node, i.e. prediction of network
