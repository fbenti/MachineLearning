...
def rlr_validate(X,y,lambdas,cvf=10):
    ...
    for train_index, test_index in CV.split(X,y):
        ...
        for l in range(0,len(lambdas)):
            model = sklearn.linear_model.Ridge(alpha=lambdas[l])
            model = model.fit(X_train,y_train)
            train_error[f,l] = np.square(y_train - model.predict(X_train)).mean(axis=0)
            test_error[f,l] = np.square(y_test - model.predict(X_test)).mean(axis=0)
            coef = model.coef_
            w[:,f,l] = coef            
